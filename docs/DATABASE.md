...
目前用于保存 `/helplink` 等全局设置，后续全局开关也可复用这张表。
...
| `local_path` | `TEXT` | 媒体文件本地相对路径，例如 `media/202601/file.jpg`。 |
...
- 本地相对路径：`media/YYYYMM/file.ext`
- 数据库存储：`media/YYYYMM/file.ext`
...
- `WEBCHAT_MEDIA_ROOT` 指向的媒体目录（默认项目根目录 `media/`）
