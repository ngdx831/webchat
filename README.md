...
```text
.
├── api_server.py       # Flask API 服务，处理网页请求、SSE 和媒体代理
├── tg_bot.py           # Telegram Bot，处理管理命令和客服回复
├── db.py               # 客服系统 SQLite 表结构和数据访问
├── config.py           # Bot、数据库、端口、媒体目录等配置
├── webchat.db          # 客服会话数据库
├── public/
...
| `ADMIN_IDS` | 初始化管理员 Telegram 用户 ID 集合；对应用户会同步为 `admin` 角色。 |
| `API_HOST` / `API_PORT` | Flask API 监听地址和端口，默认 `127.0.0.1:5055`。 |
| `DB_PATH` | 客服会话数据库路径。 |
| `WEBCHAT_MEDIA_ROOT` | 客服媒体文件落盘目录。 |
...
Telegram 用户首次发送 `/start` 或使用业务命令时会写入 `users` 表；`config.py` 里的 `ADMIN_IDS` 会自动同步为管理员。
...
管理员命令只对 `admin` 角色用户生效；`ADMIN_IDS` 会在用户初始化时自动同步为管理员。
...
| `/userkeys <telegram_user_id>` | 查看指定用户拥有的入口。 |
...
- 不要把真实 `WEBCHAT_BOT_TOKEN` 提交到公开仓库；如曾经提交过真实 Token，应立即在 BotFather 重置。
...
确认发送管理命令的 Telegram 用户是 `admin` 角色。初始管理员来自 `ADMIN_IDS`，也可以由现有管理员通过 `/userset <telegram_user_id> admin` 授权。`/valid`、`/deal` 和 `/end` 属于客服会话命令，不看管理员角色，但必须在对应客服话题内发送。
