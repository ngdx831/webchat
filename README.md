...
- 自动回复：每个 `key` 最多配置约 9 个常见问题；点击后只做自助回答，不创建客服话题。
- 会话自动过期：会话计划保留约 2 个月；本地媒体文件约 3 天后过期。
- 实时消息推送：网页端通过 SSE 接收客服回复。
- 新客呼叫状态：网页端新访客在上班状态下首次发送人工消息后，会显示「正在呼叫客服，请稍等」动画；客服回复后自动取消，3 分钟无回复时显示「客服暂时不在线，请留言」。下班状态不启用该等待动画，继续使用下班留言。
- 浏览器通知与声音：访客点击页面后会请求通知权限。页面在前台时新消息播放轻声提示音（Web Audio 生成，无外部依赖）；页面在后台且通知已授权时弹浏览器通知；通知被拒或不支持时仅播放声音。
- 支持客服媒体消息：客服回复可包含文本、图片、视频、文档和相册笔记。
- 过期媒体占位：媒体文件删除后，网页端显示「图片已过期」「视频已过期」或「文件已过期」。
- 在线/离线状态：用户用 `/kstatus` 一键切换名下所有 key 的上/下班，或 `/kstatus <key>` 切换单个 key；欢迎语和下班留言通过 `/kls → 选择 key` 内的按钮设置。
...
后端在新加坡、前端 Nginx 在香港的部署模式下，本次重构仍把媒体落在项目根目录 `media/`，访客通过 Flask `GET /api/media/<file_id>` 取媒体；将媒体迁到前端机器（挂载、HTTPS 推送或对象存储）作为后续独立优化项。
...
- Nginx 或其他反向代理，用于把公网请求转发到 Flask API。
- 客服媒体文件目录，默认是项目根目录下的 `media/`。
...
| `WEBCHAT_MEDIA_ROOT` | 客服媒体文件落盘目录，默认项目根目录 `media/`，生产可通过环境变量覆盖到其他可写目录。 |
...
| 命令 | 说明 |
| --- | --- |
| `/start` | 在主机器人显示欢迎、角色和「创建 key / 管理 key / 帮助」内联按钮；管理员还有「管理员命令」按钮和 helplink。在客户机器人则显示该 `key` 的欢迎语 + 自动回复按钮（离线时把下班留言放在欢迎语之后）。 |
| `/help` | 查看当前角色可用的帮助。 |
| `/kstatus [key]` | 切换上/下班；不带 key 时按多数决统一切换名下全部 key。 |
...
| `/botadd <key> <bot_token> [bot_username]` | 给自己的 `key` 绑定客户侧 Telegram 机器人，绑定后不需要重启 `tg_bot.py`。 |
| `/botdel <key> [bot_username]` | 删除自己入口下的客户机器人绑定。 |
| `/botls [key]` | 查看自己的客户机器人绑定。 |
| `/welcome <key> <欢迎文案>` | 设置 `key` 的欢迎语（同时影响客户机器人 `/start` 和网页打开时的首条问候）。也可在 `/kls → 选择 key → 📝 欢迎语` 按钮里设置。 |
...
| `/helplink <URL>` | 设置全局帮助链接（显示在主机器人 /start 和 /help 末尾）。 |
| `/kadd <key> <forum_chat_id> <显示名>` | 添加或更新一个网页客服入口。 |
| `/kdel <key>` | 删除客服入口。 |
| `/kstatus [key]` | 切换上/下班；不带 key 时按多数决统一切换该用户名下全部 key（欢迎语、下班留言通过 /kls 内的按钮设置）。 |
...
1. 访问 `GET /{key}?src=abc` 打开客户聊天页；iframe 挂件可使用 `GET /widget/{key}?src=abc`。
2. 聊天页调用 `GET /widget/{key}?src=abc&visitor_id=...` 获取入口配置、自动回复和来源点击统计。
3. 客户点击自动回复时，只在前端展示自助答案，不创建客服群话题。
4. 客户主动发送人工消息时，调用 `POST /api/msg/{key}` 创建或复用会话；接口返回 `created` 和 `enabled`，供前端判断是否展示新客呼叫状态。
5. 上班状态的新网页会话会展示「正在呼叫客服，请稍等」动画；SSE 收到客服回复时取消，3 分钟无回复时在当前页面提示「客服暂时不在线，请留言」。
6. 使用 `EventSource('/api/stream/{session_id}')` 接收客服回复。
7. 访客发送消息后，网页端会在浏览器支持且用户授权的情况下启用通知；页面处于后台时收到客服回复会弹出浏览器通知，历史消息不会重复通知。
8. 需要历史消息时调用 `GET /api/history/{key}?session_id=...`。
9. 客服媒体可通过 `GET /api/media/{file_id}` 获取文件；媒体过期时显示占位提示。
...
生产环境建议把 API 服务和 Bot 分成两个常驻进程，用 systemd 或 Supervisor 管理。Nginx 负责反向代理 `/<key>`、`/widget/`、`/api/` 和 `/internal/`；媒体默认由 Flask 的 `/api/media/<file_id>` 返回。
...
检查 `WEBCHAT_MEDIA_ROOT` 是否存在、进程是否有写入权限；默认通过 Flask 的 `/api/media/<file_id>` 读取媒体。
